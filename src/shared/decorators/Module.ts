...
export default function Module({ name, providers, router }: IModuleMetadata) {
  ...
  return function (constructor: Function) {
    ...
    const findSameNameModule = modulesContext.metadata.find(
      (module) => module.name === moduleName,
    );

    if (findSameNameModule)
      throw new Error(
        `Module with name ${moduleName} has already been registered.`,
      );
        ...
        for (let j = 0; j < providers.length; j++)
          if (i !== j)
            if (providers[i].provideAs === providers[j].provideAs)
              throw new Error(
                `Provider ${providers[i].provideAs} has already been registered within module ${moduleName}.`,
              );

    modulesContext.metadata.forEach((module) => {
      const registeredProviders = module.providers?.map(
        (provider) => provider.provideAs,
      );
      ...
      registeredProviders.forEach((registeredProvider) => {
        passedProviders.forEach((passedProvider) => {
          if (registeredProvider === passedProvider) {
            throw new Error(
              `The provider ${passedProvider} is already registered in the module ${module.name}.`,
            );
          }
        });
      });
